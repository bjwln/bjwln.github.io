    ...
    for block in d['blocks']:
        ...
        for line in block.get('lines', []):
            ...
            if info is None or ex.noise_text(info['text'], label, pno):
                continue
            t = info['text']
            if re.match(r'^\d+(\.\d+){2,}\s+\S', t) or re.match(r'^4\.1\.\d+\s+\S', t):
                flags = [hex(s['flags']) for s in line['spans']]
                font = [s['font'] for s in line['spans']]
                print(f'p{pno} size={info["size"]:.4f} flags={flags[:3]} font={font[:2]} {t[:90]!r}')
doc.close()
